        ...
        try:
            ...
            if scribe.user:
                ...
                if user.check_password(password):
                    ...
                    print(1)
                    return scribe_requests(request)
                else:
                    print(2)
...
def request_scribe(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(request, 'main/welcome.html', {'success': True})
        except Exception as e:
            print("Exception")
            # Pass an error message to the template
            return render(request, 'main/welcome.html', {'error': True, 'message': str(e)})
...
def accept_request(request, request_id):
    try:
        # Get the scribe request by ID
        print("In Accept Request")
        scribe_request = Request.objects.get(id=request_id)

        # Update the request status to accepted (you can add a status field to your model to track this)
        scribe_request.Is_accepted = True  # Assuming you have a 'status' field in your model
        scribe_request.save()

        # Send email to the user who made the request
        send_mail(
            'Your Scribe Request Has Been Accepted',
            f'Hello {scribe_request.full_name},\n\nYour request for a scribe has been accepted.',
            settings.DEFAULT_FROM_EMAIL,  # The email address from which the email is sent
            [scribe_request.user.email],  # The email of the user who made the request
            fail_silently=False,
        )

        # Show a success message
        messages.success(request, f"Request for {scribe_request.full_name} has been accepted. An email has been sent.")

    except Request.DoesNotExist:
        print("error in accept")
        messages.error(request, "The scribe request does not exist.")

    # Redirect back to the scribe requests page after handling the acceptance
    return redirect('scribe_requests')